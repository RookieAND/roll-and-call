...
// 다시 신청하면 문제없던 사진 URL을 그대로 옮겨 쓰므로, 이 사람의 다른 신청이 쓰는 사진은 남긴다.
...
export async function removeUnusedCertPhotos(userId: string, urls: string[]) {
  ...
  const remaining = await db
    .select({ photoUrls: certApplications.photoUrls })
    .from(certApplications)
    .where(eq(certApplications.userId, userId));
  const inUse = new Set(remaining.flatMap((row) => Object.values(row.photoUrls)));
