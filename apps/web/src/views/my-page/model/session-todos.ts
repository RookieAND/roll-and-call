...
// 세션이 열리는 것을 막는 건 이 넷뿐이라 어딘가에는 남아야 한다.
// 우선순위: ① 기한 지난 미확정(GM) ② 끝난 세션의 출석 확인(GM) ③ 신청 승인 대기(GM) ④ 가능 시간 미제출.
export function sessionTodos({ host, player }: MySessions): TodoItem[] {
  ...
  const attendance = host
    .filter((card) => card.todo?.kind === SESSION_ACTION_KIND.confirmAttendance)
    .map((card) => ({ card, eyebrow: "출석 확인이 남아 있습니다" }));
  ...
  return [...overdue, ...attendance, ...unreviewed, ...unsubmitted];
}
