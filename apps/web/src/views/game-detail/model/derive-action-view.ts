...
// 순서가 곧 우선순위: GM > 확정 > 대기 > 참여중 > 마감 > 비로그인 > 참여가능.
...
export function deriveActionView({
  ...
  if (sessionConfirmed) {
    if (viewerConfirmed) return GAME_ACTION_VIEW.confirmed;
    if (isWaiting) return GAME_ACTION_VIEW.confirmedWaiting;
    return GAME_ACTION_VIEW.closed;
  }
