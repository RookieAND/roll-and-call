...
import { Callout, HStack, SegmentedControl, Skeleton, Tabs, Text, VStack } from "@roll-and-call/ui";
      ...
      <HStack align="stretch" className="flex-1">
        <LoadingRegion label="유저 정보를 불러오는 중입니다" className="min-w-0 bg-gray-50">
          <div className="p-200 pb-150">
            <SkeletonEntity facts={["연 세션", "참여 세션", "최근 3개월 불참", "인증 룰북"]} />
          </div>
            ...
            <Tabs.Panel value={USER_DETAIL_TAB.activity} className="p-200">
              ...
              >
                <SkeletonTable
                  rows={6}
                  columns={[
                    { label: "일시", kind: "date", width: "w-[150px]" },
                    { label: "역할", kind: "badge", width: "w-[66px]" },
                    { label: "세션", kind: "text" },
                    { label: "룰북", kind: "text", width: "w-[160px]" },
                    { label: "GM", kind: "text", width: "w-[100px]" },
                    { label: "", kind: "empty", width: "w-[104px]" },
                  ]}
                />
              </Panel>
            </Tabs.Panel>
